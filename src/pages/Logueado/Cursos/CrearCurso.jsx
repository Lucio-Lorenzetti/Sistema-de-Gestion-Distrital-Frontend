...
const CrearCurso = () => {
    ...
    const guardarCurso = async () => {
        ...
        try {
            ...
            setError('No se pudo crear el curso: ' + errorMsg);
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <form ref={formRef} onSubmit={(e) => e.preventDefault()} className="flex-1 bg-scout-bg-card rounded-[2rem] border border-scout-border p-8 shadow-sm flex flex-col min-h-0 overflow-y-auto">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="flex flex-col space-y-4">
                            ...
                            {/* Lugar y Modalidad en 1 renglón */}
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    {['Programa', 'Gestion'].map((cat) => (
                                        <button key={cat} type="button" onClick={() => handleCategoriaChange(cat)} className={`flex-1 text-[10px] font-black uppercase tracking-widest px-4 py-3 rounded-xl border transition-all ${formData.categoria === cat ? 'bg-scout-primary text-white border-scout-primary' : 'bg-scout-bg-panel/50 border-scout-border'}`}>
                    ...
                    <div className="mt-8 pt-6 border-t border-scout-border flex flex-wrap items-center justify-end gap-4 shrink-0">
                        ...
                        >
                            {isLoading ? 'Guardando...' : 'Crear Curso'} <Send size={14} />
                        </button>
                    </div>
                </form>
            </div>
