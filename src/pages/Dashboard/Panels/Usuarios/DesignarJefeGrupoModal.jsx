// src/pages/Dashboard/Panels/Usuarios/DesignarJefeGrupoModal.jsx
// Designación directa (no pasa por solicitud): reemplaza al Jefe de Grupo
// actual de un grupo por el usuario elegido.
import React, { useState } from 'react';
import { X, Crown } from 'lucide-react';
import Select from '../../../../components/ui/Select';
import { useAuthorizedFetch } from '../../../../hooks/useAuthorizedFetch';

const DesignarJefeGrupoModal = ({ grupos, usuarios, grupoFijo, onClose, onDesignado }) => {
    const { authorizedFetch } = useAuthorizedFetch();
    const [grupoId, setGrupoId] = useState(grupoFijo?.id ?? '');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await authorizedFetch(`/grupos/${grupoId}/jefe-de-grupo`, {
                method: 'PATCH',
                body: { user_id: userId },
            });
            await onDesignado?.();
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
            ...
            <div className="relative bg-scout-bg-card w-full max-w-md rounded-[2rem] p-8 shadow-2xl border border-scout-border animate-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <h2 className="text-sm font-black uppercase tracking-widest text-scout-ink">Designar Jefe de Grupo</h2>
                </div>
                <p className="text-xs text-scout-muted font-medium mb-5">
                    Reemplaza al Jefe de Grupo actual de ese grupo — nunca hay dos a la vez.
                </p>
                ...
                <form onSubmit={handleSubmit}>
                    {grupoFijo ? (
                        <div className="mb-5">
                            <label className="text-[10px] font-black uppercase tracking-widest text-scout-muted mb-2 block">Grupo</label>
                            <p className="text-sm font-bold text-scout-ink">{grupoFijo.nombre}</p>
                        </div>
                    ) : (
                        <Select
                            label="Grupo"
                    ...
                    <div className="flex items-center justify-end gap-3 mt-6">
                        ...
                        <button
                            type="submit"
                            disabled={!grupoId || !userId || isSubmitting}
...
export default DesignarJefeGrupoModal;
