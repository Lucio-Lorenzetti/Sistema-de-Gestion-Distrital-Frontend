...
import { ClipboardList, CheckCircle2, Clock, Edit3, ChevronRight, Plus, Trash2 } from 'lucide-react';
import MetricCard from '../../components/ui/MetricCard';
import { useUserRole } from '../../hooks/useUserRole';
import { ROLES } from './dashboard.permissions';
import { useProgramas } from './Panels/Programas/useProgramas';
import { usePapeleraProgramas } from './Panels/Programas/usePapeleraProgramas';
import ProgramasTable from './Panels/Programas/ProgramasTable';
import PapeleraProgramasTable from './Panels/Programas/PapeleraProgramasTable';

const Programs = () => {
    const { role } = useUserRole();
    const { programas, isLoading, refetch } = useProgramas();
    const { programas: papelera, isLoading: isLoadingPapelera, refetch: refetchPapelera } = usePapeleraProgramas();
    const [vistaPapelera, setVistaPapelera] = useState(false);

    const handleRestaurado = async () => {
        await Promise.all([refetchPapelera(), refetch()]);
    };

    const handleEstadoActualizado = async () => {
        await Promise.all([refetch(), refetchPapelera()]);
    };
    ...
    return (
        ...
        >
            ...
            <div className="border-b border-scout-border pb-4 shrink-0 flex items-end justify-between flex-wrap gap-3">
                ...
                >
                    <Trash2 size={12} />
                    {vistaPapelera ? 'Ver Programas' : `Papelera${papelera.length > 0 ? ` (${papelera.length})` : ''}`}
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 gap-8 mt-10" style={{ flex: 1, minHeight: 0 }}>
                {vistaPapelera ? (
                    <PapeleraProgramasTable programas={papelera} isLoading={isLoadingPapelera} onRestaurado={handleRestaurado} />
                ) : (
                    <ProgramasTable programas={programas} isLoading={isLoading} onEstadoActualizado={handleEstadoActualizado} />
                )}
            </div>
        </div>
    );
